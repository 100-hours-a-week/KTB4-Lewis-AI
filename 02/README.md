...
또한, AI 기반 기능 요구사항을 충족하기 위해 로컬에서 구동되는 Ollama LLM(gemma4:e2b)을 연동하여 게시글 요약 및 댓글 악플 검사 기능을 추가로 구현했는데, 초기에는 요약본 전체를 한 번에 클라이언트로 전달하는 방식으로 구현했으나, 약 10초에 달하는 긴 Laytency 문제를 겪었다. 이를 해결하기 위해 FastAPI의 스트리밍(StreamingResponse) 방식으로 전환하여 청크 단위로 실시간 요약문을 전달받도록 개선했고, 요약 완료 시간을 5초 이내로 단축했다. 
